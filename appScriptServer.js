...
// var cal_id = "t3bmshdk6d6ap3grtolvqj9ppk@group.calendar.google.com"; // Dev calendar
...
// DEPOSIT
function createDeposit(json) {

	var input = JSON.parse(json);
	var name = "Deposit - " + new Date().toISOString();
	var folderId = "111aWEUhxMxk08aHz--HYE4naVfkIZbNg";
	var resource = {
		title: name,
	...
	var array = [['Date', 'Cash', 'Check', 'Credit', 'Interdepartment']];
	for (var i = 0; i < input.length; i++) {

		for (var j = 0; j < input[i].payments.length; j++) {

			if (input[i].payments[j].type === 'Cash') {
				var temp = [input[i].payments[j].date, input[i].payments[j].amount, "", "", ""];
			} else if (input[i].payments[j].type === 'Check') {
				var temp = [input[i].payments[j].date, "", input[i].payments[j].amount, "", ""];
			} else if (input[i].payments[j].type === 'Credit') {
				var temp = [input[i].payments[j].date, "", "", input[i].payments[j].amount, ""];
			} else if (input[i].payments[j].type === 'Interdepartment') {
				var temp = [input[i].payments[j].date, "", "", input[i].payments[j].amount];
			}
			array.push(temp);
	...
	sheet.getRange(1, 1, array.length, 5).setValues(array);

	var json = JSON.stringify(fileJson.alternateLink);
	database.updateData("/functions/", {
		response: json
	});
